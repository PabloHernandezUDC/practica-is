...
def getColumns(data):
    ...
    for column in data.columns:
        # para descartar strings y fechas
        if data[column].dtype not in ('StringDtype', 'datetime64'):
...
def createColumns(data, root, screen, height, width):
    ...
    choosingVariablesFrame = CTkFrame(
    ...
    xVariableScrollableFrame = CTkScrollableFrame(
        choosingVariablesFrame, orientation="horizontal", height=30, width=800)
    yVariableScrollableFrame = CTkScrollableFrame(
        choosingVariablesFrame, orientation="horizontal", height=30, width=800)
    ...
    i = 0
    for column in getColumns(data):
        CTkRadioButton(xVariableScrollableFrame, variable=v1,
                       value=i, text=column).grid(row=0, column=1+i, sticky=W)
        CTkRadioButton(yVariableScrollableFrame, variable=v2,
                       value=i, text=column).grid(row=0, column=1+i, sticky=W)
        i += 1

    choosingVariablesFrame.grid_columnconfigure(3, minsize=width*0.2)
    CTkButton(choosingVariablesFrame, text="Crear modelo y mostrar Imagen", command=lambda: makeModel(
        data, root, screen, height, width, v1, v2)).grid(row=0, column=3, rowspan=2)
...
def obtainFileForRead(width, height, root, screen):
    ...
    root.filename = filedialog.askopenfile(
        initialdir="modelos/", filetypes=tiposArchivo)
...
def readFile(width, height, root, screen, name):
    ...
    numericData = data.select_dtypes(
        include=['int16', 'int32', 'int64', 'float16', 'float32', 'float64']).head()
    ...
    for i in range(len(numericData.columns)):
        CTkLabel(dataTableScrollableFrame,
                 ...
                 justify='right',
                 # le ponemos None a la fuente para que ponga la "por defecto"
                 font=(None, 20)
...
def loadModelFromPickleObject(root, screen, height, width, name):
    ...
    equationLabel = CTkLabel(
        modelFrame, text=f"y = {round(slope, 2)} x + {round(intercept, 2)}")
    ...
    descriptionLabel = CTkLabel(
        modelFrame, text=f'Descripción: {unpickedModel.get_description()}')
